...
$koneksi = mysqli_connect("localhost","root","","website") or die(mysqli_error());
...
function tambah($koneksi){
	...
	if (isset($_POST['btn_simpan'])){
		...
		if(!empty($nama) && !empty($email) && !empty($alamat) && !empty($tgl)){
			$sql = "INSERT INTO data_user (id,nama, email, alamat, tgl) VALUES(".$id.",'".$nama."','".$email."','".$alamat."','".$tgl."')";
	...
	?> 
		<form action="" method="POST">
			<section class="base">
				...
				<label>Tanggal Lahir <input type="date" name="tgl" /></label> <br>
				<thead>
	...
	if(isset($_GET['id'])){
		?>
			...
			<table>
				...
				<label>Tanggal Lahir <input type="date" name="tgl" value="<?php echo $_GET['tgl'] ?>"/></label> <br>
				<br>
				<label>
